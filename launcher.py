...
import json
import psutil
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QProgressBar, QLineEdit, QTextEdit, QTabWidget, QGraphicsOpacityEffect, QFrame, QStackedWidget, QCheckBox, QScrollArea, QGridLayout
from PySide6.QtCore import Qt, QThread, Signal, QTimer, QProcess, QPropertyAnimation, QEasingCurve, QRect, QPoint, Property, QUrl, QParallelAnimationGroup, QSequentialAnimationGroup, QSize, QPropertyAnimation
from PySide6.QtGui import QFont, QTextCursor, QColor, QDesktopServices
CONFIG = {'base_url': 'https://raw.githubusercontent.com/LoannDev/LoannSMPLauncher/main/', 'ram_gb': 6, 'keep_launcher_open': True, 'discord_url': 'https://discord.gg/x3GtCqqXXj'}
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.log.emit(f'✅ Téléchargement terminé: {downloaded / 1048576:.2f} MB')
                    ...
                    with zipfile.ZipFile(data) as z:
                        ...
                        try:
                            ...
                            if any((v['id'] == installed for v in versions)):
                                ...
                                return
                            self.progress.emit(60, 'Installation de Minecraft + Forge (peut prendre bcp de temps)')
                            self.log.emit('\n🔨 INSTALLATION DE FORGE (peut prendre bcp de temps, c\'est littéralement minecraft)')
                            def status_cb(s):
                                if self._running:
                                    self.log.emit(s)
                            callback = {'setStatus': status_cb, 'setProgress': lambda p: None, 'setMax': lambda m: None}
...
class LauncherWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        subtitle = QLabel('100% des personnes approuvent ce launcher 👍 (j\'ai demandé à 1 personne)')
    ...
    def create_options_page(self):
        ...
        copy_logs_btn = self.create_action_button('📋 Copier les logs (mc)', self.copy_logs)
        actions_grid.addWidget(copy_logs_btn, 0, 1)
        layout.addLayout(actions_grid)
        layout.addSpacing(12)
        uninstall_label = QLabel('⚠️ ZONE DANGER ⚠️')
        ...
        hint = QLabel('* Supprime Forge, Minecraft, les mods, tout enfaite, sauf ce magnifique launcher créé par Loann Industries')
    ...
    def create_stat_card(self, title, value, color):
        # ***<module>.LauncherWindow.create_stat_card: Failure: Compilation Error
        card = QFrame()
        card.setFixedHeight(80)
        card.setStyleSheet(f'QFrame {{ background: #FFFFFF; border-left: 4px solid {color}; border-radius: 10px; }} QFrame:hover {{ background: #F8F9FA; }}')
    ...
    def on_check(self, valid):
        if valid:
            self.status.setText('✅ Tout est (normalement) à jour !')
            self.status.setStyleSheet('color: #11998E; font-weight: 600; font-size: 12px;')
            self.launch_btn.setEnabled(True)
            self.install_btn.setText('Aucune action nécéssaire')
        else:
    ...
    def launch(self):
        ...
        # ***<module>.LauncherWindow.launch: Failure: Compilation Error
        ...
        opts = {'username': user, 'uuid': '', 'token': '', 'jvmArguments': [f'-Xmx{ram}G', f'-Xms{ram // 2}G']}
        cmd = mll.command.get_minecraft_command(ver, MINECRAFT_DIR, opts)
        self.minecraft_process = QProcess(self)
        self.minecraft_process.readyReadStandardOutput.connect(lambda: logging.info(bytes(self.minecraft_process.readAllStandardOutput()).decode('utf-8', errors='ignore')))
